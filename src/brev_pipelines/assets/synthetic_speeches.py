...
def synthetic_summaries(
    ...
    # Columns for synthesis - EXCLUDE full text, INCLUDE summary
    # Summary fits in context (~2000 tokens), full text doesn't
    synthesis_columns = [
        "speech_id",
        "date",
        "central_bank",
        "speaker",
        "title",
        "summary",  # ~2000 tokens - fits in context!
        "tariff_mention",
        "is_governor",
    ]
    ...
    # Truncate summaries if needed (should already be ~2000 tokens)
    # Context budget: 2048 tokens, with 2-3 examples per prompt
    MAX_SUMMARY_LENGTH = 3000  # chars, ~750 tokens
    for record in data_for_synthesis:
        if "summary" in record and record["summary"] and len(record["summary"]) > MAX_SUMMARY_LENGTH:
    ...
    context.log.info(f"Summary truncated to {MAX_SUMMARY_LENGTH} chars if needed")
...
def synthetic_speeches(
    ...
    context.log.info(f"Expanding {total_records} synthetic summaries into full speeches...")
    ...
    for i, row in enumerate(df.iter_rows(named=True)):
        if (i + 1) % 10 == 0 or i == 0:
            context.log.info(f"Expanding speech {i + 1}/{total_records}...")

        # Build expansion prompt
        prompt = f"""You are a speechwriter for a central bank. Generate a realistic formal speech based on this summary.
...
Topic mentions tariffs: {row.get('tariff_mention', False)}

Summary:
{row.get('summary', '')}

Generate a formal central bank speech (~4000 characters) that expands on this summary.
Use appropriate economic terminology and maintain a professional, measured tone.
Include:
- Opening remarks and acknowledgments
- Main policy discussion with specific details
- Economic outlook and projections
- Concluding statements and forward guidance

Speech:"""

        # Generate expanded speech with longer timeout for 120B model
        ...
        if expanded.startswith("LLM error:"):
            context.log.warning(f"Speech {i + 1} expansion failed: {expanded}")
            # Use summary as fallback
            expanded = row.get("summary", "") or ""

        # Enforce length limit
        expanded_texts.append(expanded[:5000])

    # Add expanded text column
    df = df.with_columns(pl.Series("text", expanded_texts))

    # Update evaluation with expansion info
    evaluation_with_expansion = {
        **evaluation,
        "expansion_model": "GPT-OSS 120B",
        "expansion_target_length": TARGET_LENGTH,
        "expansion_completed_at": datetime.now(timezone.utc).isoformat(),
    }

    context.log.info(f"Expanded {len(expanded_texts)} speeches")
    avg_length = sum(len(t) for t in expanded_texts) / len(expanded_texts) if expanded_texts else 0
    context.log.info(f"Average expanded speech length: {avg_length:.0f} chars")
